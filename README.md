...
Now download Emyzelium files, say, to `~/emz-rs/`. Open `examples/demo.rs` and, right after imports, change `ALIEN_ONION` value to onion address from `/var/lib/tor/p2p_dummysite1/hostname` *without `.onion` suffix*. Also change `ALIEN_PORT` value to `60847`.
